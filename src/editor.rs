...
enum LastOperation {
    None,
    Create { path: PathBuf }, // File was created, undo is deletion
    Overwrite { path: PathBuf, original_content: Vec<u8> }, // File existed and was overwritten or modified
}
...
impl Editor {
    pub fn new() -> Self {
        Editor { last_op: LastOperation::None }
    }
    ...
    fn record_write_op(&mut self, path: &Path, original_content: Option<Vec<u8>>) {
        if let Some(content) = original_content {
            ...
            self.last_op = LastOperation::Create { path: path.to_path_buf() };
        }
    }
}
...
pub fn handle_command(editor: &mut Editor, args: EditorArgs) -> Result<Option<String>, String> {
    ...
    match args.command {
        ...
        CommandType::Create => {
            let content = args.file_text.ok_or_else(|| "Error: 'file_text' is required for 'create' command.".to_string())?;
            create_file(editor, &path, &content)
        }
        CommandType::StrReplace => {
            let old_s = args.old_str.ok_or_else(|| "Error: 'old_str' is required for 'str_replace' command.".to_string())?;
            let new_s = args.new_str.unwrap_or_default();
            str_replace_in_file(editor, &path, &old_s, &new_s)
        }
        CommandType::Insert => {
            let line_num_1_indexed = args.insert_line.ok_or_else(|| "Error: 'insert_line' is required for 'insert' command.".to_string())?;
            if line_num_1_indexed == 0 {
                return Err("Error: 'insert_line' must be 1-indexed and positive.".to_string());
            }
            let new_s = args.new_str.ok_or_else(|| "Error: 'new_str' is required for 'insert' command.".to_string())?;
    ...
    match view_range {
        Some(range) => {
            ...
            if total_lines == 0 { // Empty file
                ...
                } else if start_line == 1 && end_line < 1 && end_line != -1 { // e.g. [1,0]
                     return Err(format!("Error: End line {} is invalid for start line {} on an empty file.", end_line, start_line));
                }
                // start_line > 1 on empty file, or invalid end_line for start_line 1
                return Err(format!("Error: Start line {} is beyond the end of an empty file or range is invalid.", start_line));
            }

            // File has content
            if start_line > total_lines {
                return Err(format!("Error: Start line {} is beyond the end of file ({} lines).", start_line, total_lines));
            }
            ...
            if end_line == -1 {
                ...
                return Err(format!("Error: End line {} cannot be less than start line {}.", end_line, start_line));
            }
...
fn create_file(editor: &mut Editor, path: &Path, content: &str) -> Result<Option<String>, String> {
    let original_content = if path.exists() {
        if path.is_dir() {
            return Err(format!("Error: Path '{}' is a directory, cannot create file.", path.display()));
        }
        Some(fs::read(path).map_err(|e| format!("Error reading existing file '{}' for undo: {}", path.display(), e))?)
    } else {
        None
    };
...
fn str_replace_in_file(editor: &mut Editor, path: &Path, old_str: &str, new_str: &str) -> Result<Option<String>, String> {
    if !path.exists() {
        return Err(format!("Error: File not found at '{}'", path.display()));
    }
    if !path.is_file() {
    ...
    let original_content_bytes = fs::read(path)
        .map_err(|e| format!("Error reading file '{}': {}", path.display(), e))?;
...
fn insert_into_file(editor: &mut Editor, path: &Path, insert_line_0_indexed: usize, text_to_insert: &str) -> Result<Option<String>, String> {
    if !path.exists() {
        return Err(format!("Error: File not found at '{}' for insert operation.", path.display()));
    }
    if !path.is_file() {
        return Err(format!("Error: Path '{}' is not a file.", path.display()));
    }
    ...
    if insert_line_0_indexed > lines.len() { // e.g. 3 lines (len=3, idx 0,1,2). insert_0_idx=3 (after line 3 / at line 4). This is an append.
    ...
    if lines.is_empty() && insert_line_0_indexed == 0 { // Inserting into an empty file at (1-indexed) line 1
        lines.push(text_to_insert.to_string());
    } else if insert_line_0_indexed == lines.len() { // Append: insert_line (1-idx) is 1 greater than total lines
        lines.push(text_to_insert.to_string());
    } else { // Insert in the middle: insert_line_0_indexed < lines.len()
    ...
    if !original_content_str.is_empty() && original_content_str.ends_with('\n') && !lines.is_empty() && !modified_content.ends_with('\n') {
        modified_content.push('\n');
    }
    // If original was empty, or didn't end with newline, and new content (from single line insert) doesn't have one, it's fine.

    if modified_content != original_content_str { // Compare with string representation, not bytes, due to potential newline char differences
        fs::write(path, &modified_content)
            .map_err(|e| format!("Error writing to file '{}': {}", path.display(), e))?;
        editor.record_write_op(path, Some(original_content_bytes));
    }
    ...
    match std::mem::replace(&mut editor.last_op, LastOperation::None) {
        ...
        LastOperation::Create { path } => {
            if path.exists() && path.is_file() {
                fs::remove_file(&path).map_err(|e| format!("Error undoing creation (deleting file '{}'): {}", path.display(), e))?;
            }
            // If not exists or not a file, consider undo successful as the state (no file) is achieved.
            Ok(None)
        }
        LastOperation::Overwrite { path, original_content } => {
            if path.is_dir() {
                 // editor.last_op is already None, must restore it if op fails early
                 editor.last_op = LastOperation::Overwrite{path: path.clone(), original_content};
                 return Err(format!("Error undoing overwrite: Path '{}' is a directory.", path.display()));
            }
            fs::write(&path, original_content)
                .map_err(|e| {
                    ...
                    format!("Error undoing overwrite (writing original content to '{}'): {}", path.display(), e)
                })?;
            Ok(None)
        }
    }
...
mod tests {
    ...
    fn test_str_replace_and_undo() {
        ...
        assert_eq!(fs::read_to_string(&file_path).unwrap(), "bye world, bye moon");

        // Undo Replace
        let undo_args = make_args_struct(CommandType::UndoEdit, file_path_str);
        handle_command(&mut editor, undo_args).unwrap();
        assert_eq!(fs::read_to_string(&file_path).unwrap(), "hello world, hello moon");
    }
    ...
    fn test_view_ranges_detailed() {
        ...
        let test_cases = vec![
            ...
            (Some(vec![6, 7]), Err("Start line 6 is beyond the end of file (5 lines)")), // Start out of bounds
            (Some(vec![0, 2]), Err("Start line in 'view_range' must be positive")),
            (Some(vec![2, 0]), Err("End line in 'view_range' cannot be 0")),
            (Some(vec![3, 2]), Err("End line 2 cannot be less than start line 3")),
            (Some(vec![1,2,3]), Err("'view_range' must contain exactly two elements")),
        ];
        ...
        for (range, expected) in test_cases {
            ...
            match expected {
                Ok(exp_str) => assert_eq!(result.unwrap().unwrap(), exp_str, "Mismatch for range {:?}", range),
                Err(err_msg_part) => {
                    let err = result.unwrap_err();
                    assert!(err.contains(err_msg_part), "Expected error containing '{}' for range {:?}, got '{}'", err_msg_part, range, err);
                }
            }
        }
        ...
        assert_eq!(handle_command(&mut editor, args_empty.clone()).unwrap().unwrap(), "");
        
        args_empty.view_range = Some(vec![1,-1]);
        assert_eq!(handle_command(&mut editor, args_empty.clone()).unwrap().unwrap(), "");

        args_empty.view_range = Some(vec![2,2]);
        assert!(handle_command(&mut editor, args_empty.clone()).unwrap_err().contains("Start line 2 is beyond the end of an empty file"));
    }
    ...
    fn test_insert_into_empty_file_and_append() {
        ...
        let mut args = EditorArgs {
            insert_line: Some(1), new_str: Some("First Line".to_string()),
    ...
    fn test_insert_error_conditions() {
        ...
        // insert_line 0
        let mut args = EditorArgs {
            insert_line: Some(0), new_str: Some("fail".to_string()),
            ..make_args_struct(CommandType::Insert, path_str)
        };
        assert!(handle_command(&mut editor, args.clone()).unwrap_err().contains("'insert_line' must be 1-indexed"));
        ...
        assert!(handle_command(&mut editor, args.clone()).unwrap_err().contains("File not found"));
    }
    ...
    fn test_str_replace_no_change() {
        ...
        editor.last_op = LastOperation::Create { path: PathBuf::from("dummy") };
